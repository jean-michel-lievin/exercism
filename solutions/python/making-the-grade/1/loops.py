...
def letter_grades(highest):
    ...
    failing = 40
    step = (highest - 40) // 4
    return [failing + 1 + (step*i) for i in range (4)]
...
def student_ranking(student_scores, student_names):
    ...
    """
    return [f"{idx+1}. {name}: {score}" for idx, (score, name) in enumerate(sorted(zip(student_scores, student_names), reverse=True))]
